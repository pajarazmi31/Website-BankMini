...
<div id="viewTabelData" class="fade-in flex flex-1 flex-col justify-start">
    <!-- Search Bar Mobile -->
    <form action="{{ route('supervisor.datanasabah') }}" method="get" class="md:hidden relative mb-5">
    ...
    <div class="bg-white rounded-[20px] shadow-card p-6 w-full flex flex-col">
        <div class="flex justify-between items-center mb-4 border-b border-gray-50 pb-2">
            <form action="{{ route('supervisor.datanasabah') }}" method="get" class="hidden md:flex gap-2 items-center">
                <div class="relative">
                    <i class="ph ph-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-lg"></i>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    ...
                    <tbody class="text-[14px] text-gray-800 font-medium">
                        @forelse ( $userNasabah as $index => $nasabah )
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="py-4 px-2 border-b border-gray-50 pl-4">{{ $userNasabah->firstItem() + $index }}.</td>
                                ...
                                <td class="py-4 px-4 border-b border-gray-50">{{ $nasabah->rekening->id ?? 'Belum Ada Akun' }}</td>
                        ...
                        @empty
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="py-4 px-2 border-b text-gray-500 border-gray-50 text-center pl-4" colspan="6">Maaf Data Kosong</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
